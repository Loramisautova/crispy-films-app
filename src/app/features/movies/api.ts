...
import { IPaginatedMovieList, IMovieListItem, IPeopleBase, ICast, ICrew, IMovieCreditList } from '../models';
...
export const tmdbMoviesApi = createApi({
    ...
    endpoints: (build) => ({
        ...
        getCredits: build.query<IMovieCreditList, string>({
            query: (movieId) => ({
                url: `/${movieId}/credits`,
                params: { api_key: '2982bad10a93c3bc7f2c5245f865294c' },
            }),
            transformResponse: (response: Record<string, unknown>): IMovieCreditList => transformToCamelCase(response),
        }),
    }),
});

// eslint-disable-next-line @typescript-eslint/unbound-method
export const { useGetPopularMoviesQuery, usePrefetch, useGetMovieQuery, useGetCreditsQuery } = tmdbMoviesApi;
